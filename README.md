...
**Day-ahead electricity load forecasting for residential homes**
*MSc AI Thesis (NCI, AICS 2025, Springer CCIS) → Pre-incorporated Irish Cleantech Startup*
...
## Quick Start — Local Stack (Mac Mini / Docker)

```bash
cp env.example .env           # fill in DB_PASSWORD and GRAFANA_PASSWORD
docker compose up -d          # start FastAPI + PostgreSQL + Redis + Grafana + Caddy
open https://api.sparc.localhost/health   # API health check (run: caddy trust first)
open https://grafana.sparc.localhost      # Grafana operator dashboard (admin)
```

Or run just the ML pipeline without Docker:

```bash
pip install -e ".[dev]"
python scripts/run_pipeline.py --city ireland --save-predictions
python deployment/live_inference.py --dry-run    # morning brief CLI
```
...
### Production Stack (Phase 6/7 — 2026)

```
Consumer Layer (D-23 — in progress)
  Next.js 15 PWA + Tremor charts + shadcn/ui
      ↓ HTTPS via Caddy / Cloudflare Tunnel
API Layer
  FastAPI + APScheduler (daily 16:00 batch)
  Endpoints: /predict · /control · /health (drift status)
      ↓
ML Layer
  LightGBM H+24 (ModelRegistry: CANDIDATE→ACTIVE→RETIRED)
  DriftDetector (KS + PSI) · ControlEngine (DEFER_HEATING / HEAT_NOW / ALERT_HIGH_DEMAND)
  Audit log: outputs/logs/control_decisions.jsonl (EU AI Act Art. 52)
      ↓
Data Layer
  PostgreSQL 16 + TimescaleDB (households · predictions · recommendations · outcomes)
  Redis (prediction cache TTL 23h)
      ↓
Operator Layer (internal)
  Grafana OSS · Streamlit (analytics/research)

Local hosting: docker compose up -d (Mac Mini M5)
Production:    AWS App Runner + RDS + Vercel (Phase 3)
```

### Research Pipeline (MSc 2025 — Paradigm Parity Experiment)
...
### Docker Compose (Phase 1 — Mac Mini M5)

```bash
cp env.example .env          # set DB_PASSWORD, GRAFANA_PASSWORD, MYENERGI_API_KEY
docker compose up -d         # starts: FastAPI · PostgreSQL · Redis · Grafana · Caddy
docker compose logs -f api   # tail API logs
```

Services: `api` (8000) · `db` (5432 — TimescaleDB) · `redis` (6379) · `grafana` (3001) · `caddy` (443)

### AWS App Runner (Phase 3 — production)
...
|------------|--------|-------|
| **myenergi Eddi** | ✅ Live | `get_status()`, `get_schedule()`, `get_history_day()` — HTTP Digest Auth |
| **Open-Meteo** | ✅ Live | Free weather + solar forecast — no API key, retry + stale-cache fallback |
| **ESB Networks HDF** | ✅ Live | Irish smart meter CSV upload (manual MVP; SMDS automatic access mid-2026) |
| **SEMO / ENTSO-E** | 🔴 Stub | Day-ahead market prices — implementation pending (D-20) |
...
The CI pipeline (GitHub Actions) runs all 153 tests against Python 3.11 and 3.12 on every push. GPU is not required; all models support CPU training.

---

## Active Development

See [`ROADMAP.md`](ROADMAP.md) for the full 5-track product engineering roadmap (Research · Engineering · Product · Deployment · Commercial).

**Completed since conference paper:**
- ✅ H+24 Day-Ahead Evaluation — 3-Way Paradigm Parity (Setup A/B/C); LightGBM MAE 4.029, PatchTST MAE 6.955
- ✅ Oslo Generalisation — all tree models R² > 0.95 on 48-building dataset
- ✅ Out-of-fold stacking — 5-fold TimeSeriesSplit, gap=168h, Ridge meta-learner
- ✅ Probabilistic forecasting — LightGBM quantile objective, P10/P50/P90
- ✅ ModelRegistry — CANDIDATE→ACTIVE→RETIRED lifecycle, rollback, regression gate
- ✅ DriftDetector — KS test + PSI per feature, rolling MAE trigger (7d window)
- ✅ ControlEngine — demand-response with JSONL audit log (EU AI Act Art. 52)
- ✅ Home trial live — €178/yr saving identified on Irish residential ESB meter
- ✅ Docker Compose local stack — full production infrastructure on Mac Mini M5

**Next milestones:**
- 📝 Applied Energy journal submission (H+24 Paradigm Parity)
- 🔧 E-17: `run_pipeline.py` SRP refactor
- 🔧 E-19: Strict Pydantic schemas for FastAPI
- 🏠 D-23: Next.js PWA consumer app
- ⚡ P-02: WhatsApp morning brief delivery
...
```bash
pip install -e ".[dev]"
pytest tests/ -v --cov=src/energy_forecast   # 153 tests
```
