...
THESIS_METRICS = pd.DataFrame(
    [
        ...
        {"Model": "Stacking Ensemble (LGBM meta)", "MAE": 3.582, "RMSE": 7.030, "R²": 0.9782},
        {"Model": "Stacking Ensemble (Ridge meta)", "MAE": 3.698, "RMSE": 7.051, "R²": 0.9781},
...
def _load_pipeline_metrics(results_dir: Path) -> pd.DataFrame | None:
    ...
    col_map = {
        c: c.replace("MAE", "MAE").replace("RMSE", "RMSE").replace("R2", "R²").replace("R²", "R²")
        for c in df.columns
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Skip heavy charts (ACF, decomposition, profiles)"
    )
