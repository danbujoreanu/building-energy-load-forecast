...
def build_raw_sequences(
    ...
    X_v_scaled = scaler_X.transform(df_val[feature_cols].values)
    y_v_scaled = scaler_y.transform(df_val[[target_col]].values).squeeze()
    
    X_te_scaled = scaler_X.transform(df_test[feature_cols].values)
    y_te_scaled = scaler_y.transform(df_test[[target_col]].values).squeeze()
    
    def _build_sequences_for_split(X_scaled: np.ndarray, y_scaled: np.ndarray, df_index: pd.MultiIndex):
        X_seqs, y_seqs = [], []
        ...
        df_X = pd.DataFrame(X_scaled, index=df_index, columns=feature_cols)
        ...
        for bid in building_ids:
            ...
            X_b = df_X.xs(bid, level="building_id").values
    ...
    X_train_seq, y_train_seq = _build_sequences_for_split(X_tr_scaled, y_tr_scaled, df_train.index)
    X_val_seq, y_val_seq = _build_sequences_for_split(X_v_scaled, y_v_scaled, df_val.index)
    X_test_seq, y_test_seq = _build_sequences_for_split(X_te_scaled, y_te_scaled, df_test.index)
