# Rory Design Principle — UX Voice & Tone Standard
...
**Rory** is a fictional Irish household member — the non-technical user whose experience defines every copy decision.

- **Age:** 38. Lives in Maynooth with a partner. Has solar panels, doesn't know what kWh means.
...
Rory is **not** an engineer. Rory is **not** Dan.
...
The action only makes sense when Rory understands the context. Lead with the reason.
...
Any message asking Rory to act must have a visible decline path — "No thanks", "Dismiss", or "Remind me later". Invisible auto-actions are only permitted for zero-comfort-impact events (solar diversion).
...
| Scenario | Example (Rory-compliant) |
|----------|--------------------------|
...
| Product | Gate new features on Rory copy review before ship |
