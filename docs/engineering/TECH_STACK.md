...
CREATE TABLE recommendations (
    ...
    reasoning       TEXT,
    user_message    TEXT,                            -- plain English for non-technical users (see UX_DESIGN_PRINCIPLES.md)
    p50_kwh         NUMERIC(8,4),
...
## 8. Notifications — n8n + Pushover (Phase 1 Live) → WhatsApp Business API (Phase 3)

### Phase 1 (Current — Mac Mini, single user): n8n + Pushover

**n8n** (localhost:5678, running in Docker Compose) handles all notification workflow logic:

| Workflow | Trigger | Delivery | Status |
|----------|---------|----------|--------|
| Daily Morning Brief | 08:00 cron → `/control` | Pushover push | ✅ Live |
| Grafana Alert Relay | Grafana webhook | Pushover push (priority HIGH) | ✅ Live |
| Weekly Drift Check | Mon 09:00 → `/health` | Pushover if CRITICAL | ✅ Live |

**Pushover** (pushover.net, €4.99 one-time) — iOS/Android native push app:
- App token: stored in `.env` as `PUSHOVER_APP_TOKEN`
- User key: stored in `.env` as `PUSHOVER_USER_KEY`
- Delivery: < 3 seconds, works even when WhatsApp is unavailable
- Priority 2 (emergency) for deadline alerts — requires acknowledgement

**CallMeBot WhatsApp** — backup channel, credentials in `.env`.

Full workflow reference: `docs/infra/services/N8N_WORKFLOWS.md`

### Phase 3 (Scale — 1000+ households): WhatsApp Business API

| Channel | Tool | Cost | When |
|---------|------|------|------|
| WhatsApp | Meta WhatsApp Business API via Twilio | ~€0.03/message | Phase 3 |
| Email | Resend (modern SendGrid replacement) | Free: 3k/month | Phase 2 |
| Push (web) | Web Push API (built into Next.js PWA) | Free | Phase 2 |
| SMS | Twilio SMS | ~€0.04/SMS | Fallback only |

**Implementation note:** The morning brief content is already generated by `deployment/live_inference.py`. n8n and future channels are delivery adapters — they consume the same structured JSON output from `/control`.

### Slack (connected to Linear — 2026-05-01, not actively used)

Linear is connected to a Slack workspace. Current use: Linear posts issue activity to a Slack channel (low-value solo).

**Potential future use cases (not yet built — no Linear issue):**
1. **n8n → Slack webhook as second alert channel** — backup to Pushover if the app is down. Build time: ~20 min in n8n.
2. **Slack bot querying `/forecast`** — "Hey Sparc, what's my predicted bill tomorrow?" → n8n Slack Events API webhook → calls API → posts reply. Good product demo for PhD/investor conversations.
3. **Team collab** — when there's a co-founder or beta customer, Slack + Linear is standard developer workflow.

**When to act on Slack:** when there's a second person (co-founder/hire) or when a Slack demo is needed for a pitch/interview. Not worth building solo.

APIs reference: https://docs.slack.dev/apis/
