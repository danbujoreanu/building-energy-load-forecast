...
**Setup C on Oslo — paradigm parity confirmed cross-city:** PatchTST (Setup C) was also evaluated on the Oslo dataset using the identical raw-sequence architecture (72-hour lookback, 24-hour horizon). Oslo PatchTST achieves MAE = 13.616 kWh and R² = 0.8741, compared to LightGBM (Setup A) MAE = 7.415 kWh, R² = 0.963. This represents an **84% relative MAE disadvantage for PatchTST on Oslo** — a larger gap than on Drammen (42%), and consistent in direction. Notably, Oslo PatchTST (R² = 0.874) falls below the linear Ridge baseline on Drammen (R² = 0.926), confirming that the raw-sequence paradigm does not generalise as effectively as engineered tabular features across the two portfolios.

**Table 9: Cross-City Paradigm Comparison (H+24, Setup A vs Setup C)**

| City | Model | Setup | MAE (kWh) | R² | MAE gap vs LightGBM |
|------|-------|-------|-----------|-----|---------------------|
| Drammen | LightGBM | A | 4.029 | 0.9752 | — |
| Drammen | PatchTST | C | 6.955 | 0.9102 | +72% |
| Oslo | LightGBM | A | 7.415 | 0.9630 | — |
| Oslo | PatchTST | C | 13.616 | 0.8741 | +84% |

The widening gap on Oslo (84% vs 72%) is consistent with the hypothesis that PatchTST's performance is more sensitive to the training corpus size and temporal homogeneity of buildings. Oslo schools, while numerous (48 buildings), have more uniform usage profiles (school schedule-dominated) than the heterogeneous Drammen portfolio (offices, schools, nurseries, nursing homes). Paradoxically, this homogeneity does not benefit PatchTST — it may reflect that the 72-hour raw lookback cannot capture the longer-range weekly occupancy patterns (lag_168h ≈ 35% SHAP importance) that tabular features encode explicitly.
...
## 7. Responsible AI, Ethics, and Deployment Governance

### 7.1 Explainability and Transparency

The adoption of AI forecasting systems in energy management raises obligations of transparency that go beyond algorithmic performance. End users — building managers, facility operators, and households — must be able to understand and contest forecasting decisions that affect billing, scheduling, or demand-response actions.

This system addresses explainability at three levels. At the **model level**, LightGBM's gradient-boosted tree structure permits exact feature attribution via SHAP (SHapley Additive exPlanations) [3]. SHAP beeswarm analysis across the 44-building Drammen test set identifies a consistent top-3 feature importance ordering — lag_168h (~35%), lag_24h (~20%), rolling_mean_168h (~12%) — that aligns with physical intuition: buildings at the same time last week, and the running average trend, are the primary predictors. This attribution can be surfaced to operators as natural-language explanations ("Today's forecast is high because consumption was above average this week").

At the **decision level**, the control system translates probabilistic forecasts into discrete, auditable actions (HEAT_NOW, DEFER_TO_SOLAR, ADVANCE_TO_OFFPEAK, ALERT_HIGH_DEMAND) with a human-readable `reason` string logged per action. No action is taken without explicit operator visibility. The system is designed as a **decision support tool** — recommendations are presented, not automatically executed, unless the user has explicitly opted into automated setpoint control.

At the **system level**, all model weights, training data provenance, feature engineering pipeline, and evaluation metrics are version-controlled in a public repository. The COFACTOR and Oslo datasets have citable DOIs. This supports independent reproducibility and audit.

### 7.2 Privacy and Data Minimisation

Smart meter data at 30-minute (Irish ESB) or hourly (Norwegian COFACTOR) resolution reveals occupancy patterns, daily routines, and behavioural irregularities sufficient to infer sensitive household information — whether occupants are at home, work schedule, and physical activity patterns. At 1-minute resolution (Eddi API), appliance-level disaggregation is possible.

The deployment architecture applies the following mitigations:

- **Data minimisation**: Only pre-computed statistical features (lag values, rolling means) are transmitted to the inference endpoint. Raw interval data never leaves the user's local environment during inference.
- **Jurisdiction**: All cloud components are hosted in `eu-west-1` (Ireland), ensuring GDPR Art. 44 adequacy. No raw consumption data is transmitted to third-party LLM APIs; only aggregate statistics are used.
- **Consent and transparency**: The privacy policy explicitly discloses that 30-minute interval data enables occupancy inference. Users are informed at onboarding that this is a security risk and that on-device inference is the recommended long-term architecture.
- **Retention limits**: Raw interval data is retained for 24 months (the minimum required for the rolling-window retraining schedule). Older data is purged automatically.

These mitigations are consistent with GDPR Art. 5(1)(b) (purpose limitation) and Art. 5(1)(c) (data minimisation).

### 7.3 Fairness and Access

Energy forecasting systems that are deployed only to users with smart meters, high-resolution devices, or cloud connectivity risk exacerbating existing energy inequality. Users without smart meters, those on prepay plans, or those with poor connectivity are excluded from algorithmic optimisation benefits.

The tiered data access architecture (Tier 1: manual CSV upload; Tier 2: cloud API history pull; Tier 3: nightly cron; Tier 4: hardware P1 adapter) is explicitly designed to serve Tier 1 users — the lowest technical barrier — with meaningful forecasts from a single ESB CSV export. The production model (LightGBM with 35 features) requires no real-time connectivity and can generate a full 24-hour forecast from a file uploaded once per month.

### 7.4 Model Governance and Drift Monitoring

Production deployment introduces temporal distribution shift: building usage patterns change with occupancy, renovation, and tariff regime changes. A model trained on 2018–2022 Norwegian data will encounter distribution shift when applied to 2026 data.

The governance framework specifies:
- **Daily monitoring**: rolling 7-day MAE computed against observed consumption. Alert threshold: 7d MAE > 1.5× training MAE.
- **Revenue-denominated alerts**: forecast errors are quantified in euros (MAE × electricity price), not abstract kWh units, providing operators with actionable cost context.
- **Monthly retraining**: rolling 24-month window. Drift trigger activates immediate retraining outside the monthly cycle.
- **Feedback signal discipline**: observed consumption is the only feedback signal used for retraining. User action compliance (whether a demand-response recommendation was followed) is explicitly excluded — training on compliance introduces selection bias and gamification risk.
- **Human override**: all automated setpoint changes can be overridden or suppressed by the user at any time. The system reduces to display-only mode if the user disables automated control.

### 7.5 Responsible Use of AI-Assisted Development

Following Howard [31], the development of this system prioritised domain understanding over code volume. Every component — the feature engineering pipeline, the control engine, the connector layer — was designed with explicit specifications before implementation, and the team retains full understanding of all code paths. Key safeguards implemented:

- No component was deployed based on "it works in testing" alone without explicit edge-case analysis.
- The Eddi API connector includes explicit fallback paths, error logging, and fails safely (no setpoint changes on API error).
- The control engine is a pure function (`schedule_advice`) with no side effects — it can be tested, audited, and reasoned about independently of the API layer.
- Known limitations are documented: history API unit ambiguity, no temperature probe, NWP weather oracle gap. These are explicit caveats in the codebase, not silent assumptions.
...
[31] Howard, J. (2026). The Dangerous Illusion of AI Coding. *Machine Learning Street Talk Podcast Transcript*, March 2026. [Podcast transcript]
