...
from energy_forecast.utils import load_config
from energy_forecast.evaluation.metrics import evaluate, save_per_building_metrics
...
def main():
    ...
    for city in cities:
        ...
        # Load Splits
        try:
            X_test = pd.read_parquet(proc_dir / f"{city}_X_test_fs.parquet")
            y_test = pd.read_parquet(proc_dir / f"{city}_y_test.parquet").squeeze()
            X_train = pd.read_parquet(proc_dir / f"{city}_X_train_fs.parquet")
            y_train = pd.read_parquet(proc_dir / f"{city}_y_train.parquet").squeeze()
            X_val = pd.read_parquet(proc_dir / f"{city}_X_val_fs.parquet")
            y_val = pd.read_parquet(proc_dir / f"{city}_y_val.parquet").squeeze()
        ...
        for m_path in sklearn_paths:
            ...
            if "_meta" in fname or "Quantile" in fname or "scaler" in fname: continue
            mname = fname.replace(f"{city}_", "").split("_")[0]
            if any(r["model"] == mname for r in results): continue
            try:
        ...
        for mname in dl_targets:
            ...
            if not match: match = [p for p in keras_paths if mname in Path(p).name and city not in Path(p).name]
            if not match: continue
            
            try:
                model = tf.keras.models.load_model(match[0], compile=False)
                X_seq, _ = build_sequences(X_test, y_test, lookback, horizon)
