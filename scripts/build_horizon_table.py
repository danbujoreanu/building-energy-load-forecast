...
# Display order
DISPLAY_ORDER = [
    "LightGBM", "XGBoost", "RandomForest", "Ridge", "Lasso",
    "Stacking",
    "CNN-LSTM", "GRU", "LSTM",
    "Mean Baseline",
]
...
def print_table(df: pd.DataFrame) -> None:
    ...
    print("=" * 100)
    print(f"{'Model':<22} {'Setup':<5} {'H+1 MAE':>8} {'H+1 R²':>8} {'H+24 MAE':>9} "
    ...
    for _, row in df.iterrows():
        ...
        degrad  = f"{row['degradation_factor']:.2f}×" if pd.notna(row["degradation_factor"]) else "  —  "
        note    = row["note"][:40] if row["note"] else ""
        print(f"{row['model']:<22} {row['setup']:<5} {h1_mae:>8} {h1_r2:>8} "
    ...
    dl_dg_sane = df.loc[dl_mask & df["degradation_factor"].notna() & (df["degradation_factor"] < 5),
                         "degradation_factor"]

    print(f"\nSetup A (Trees):      degradation range {tree_dg.min():.2f}× – {tree_dg.max():.2f}×  "
          f"(mean {tree_dg.mean():.2f}×)")
    print(f"Setup B (DL, sane):   degradation range {dl_dg_sane.min():.2f}× – {dl_dg_sane.max():.2f}×  "
          f"(mean {dl_dg_sane.mean():.2f}×)  [LSTM convergence failure excluded]")
    print(f"LSTM at H+24:         {df.loc[df['model'] == 'LSTM', 'degradation_factor'].values[0]:.1f}×  "
