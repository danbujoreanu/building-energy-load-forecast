...
def fig1_paradigm_parity() -> None:
    ...
    sel["_r2"] = (
        sel["R²"].fillna(sel["R2"])
        if ("R²" in sel.columns and "R2" in sel.columns)
    ...
    bars = ax.barh(
        y_pos, display_mae, color=colors, edgecolor="white", linewidth=0.5, height=0.72, zorder=3
    )
    ...
    for idx, (yp, p) in enumerate(rows_with_paradigm):
        if p != prev_p:
            if prev_p is not None:
                ax.axhspan(
                    start_i - 0.5, yp - 0.5, facecolor=bg_colors[prev_p], alpha=0.45, zorder=1
                )
                group_label_pos[prev_p] = (start_i + yp - 1) / 2
            start_i = yp
            prev_p = p
    # Last group
    ax.axhspan(start_i - 0.5, y_pos[-1] + 0.5, facecolor=bg_colors[prev_p], alpha=0.45, zorder=1)
    ...
    ax.set_xlabel(
        "MAE (kWh)  [bars truncated at 14 kWh; actual values annotated with →]",
        fontweight="bold",
        fontsize=9,
    )
    ...
    legend_patches = [
        ...
        mpatches.Patch(
            color=PALETTE["B"], label="Setup B — DL on engineered features (negative control)"
        ),
...
def fig2_ensemble_blend() -> None:
    ...
    fig.suptitle(
        "Grand Ensemble: Weighted Average of Setup A + Setup C Predictions\n"
...
def fig3_oslo_generalisation() -> None:
    ...
    label_map = {
        ...
        "Ridge": "Ridge",
        "Lasso": "Lasso",
        "LightGBM_Quantile": "LGBM Quantile",
        "Mean Baseline": "Mean Baseline",
    }
    ...
    d_mae = [
        (
            combined[(combined["model"] == m) & (combined["city"] == "Drammen")]["MAE"].values[0]
    ...
    o_mae = [
        (
            combined[(combined["model"] == m) & (combined["city"] == "Oslo")]["MAE"].values[0]
    ...
    d_r2 = [
        (
            combined[(combined["model"] == m) & (combined["city"] == "Drammen")]["R2"].values[0]
            if len(combined[(combined["model"] == m) & (combined["city"] == "Drammen")]) > 0
            else np.nan
        )
        for m in models_ordered
    ...
    o_r2 = [
        (
            combined[(combined["model"] == m) & (combined["city"] == "Oslo")]["R2"].values[0]
            if len(combined[(combined["model"] == m) & (combined["city"] == "Oslo")]) > 0
            else np.nan
        )
        for m in models_ordered
    ...
    fig.suptitle(
        "Setup A Pipeline: Zero-Shot Generalisation Drammen → Oslo\n"
...
def fig4_quantile_calibration() -> None:
    ...
    fig.suptitle(
        "LightGBM Quantile Forecaster (P10/P50/P90) — Probabilistic Evaluation\n"
...
def fig5_per_horizon_mae() -> None:
    ...
    for _, row in dl_rows.iterrows():
        try:
            ...
            setup = (
                "B"
                if "SetupB" in str(row["model"])
                else "C" if "SetupC" in str(row["model"]) else "B"
            )
            label_name = str(row["model"]).replace("_SetupB", " (B)").replace("_SetupC", " (C)")
    ...
    ax.set_title(
        "Per-Horizon MAE across the 24-Hour Prediction Window\n"
        "DL models vs LightGBM Setup A reference",
        fontweight="bold",
    )
...
def fig6_methodology_overview() -> None:
    ...
    # ── Helper: rounded box ────────────────────────────────────────────────
    def box(cx, cy, w, h, text, color="#2c7bb6", text_color="white", fontsize=9, bold=False):
    ...
    box(12.0, 1.8, 1.8, 0.7, "PatchTST: R²=0.910\nMAE=6.96 kWh", "#e6f4e0", "#4dac26", 8)
    ...
    box(
        ...
        3.5,
        0.6,
        "Grand Ensemble  α·A + (1−α)·C  →  best at α=1.0 (pure A)",
        PALETTE["Ensemble"],
        "white",
        8,
        True,
    ...
    ax.text(
        6.5,
        6.75,
        "Three-Paradigm Pipeline Architecture",
        ha="center",
        va="center",
        fontsize=14,
        fontweight="bold",
    ...
    ax.text(
        6.5,
        6.35,
        "Municipal Buildings (H+24 Day-Ahead Forecasting)",
        ha="center",
        va="center",
        fontsize=10,
        color="#555555",
...
def fig7_horizon_sensitivity() -> None:
    ...
    models = ["LightGBM", "XGBoost", "RF", "Ridge", "Lasso", "CNN-LSTM", "GRU", "LSTM"]
    labels = [
        "LightGBM ★",
        "XGBoost",
        "Random Forest ★",
    ...
    ax.text(
        2.0,
        Y_MAX * 0.96,
        "Setup A — Trees + Engineered Features",
        ha="center",
        va="top",
        fontsize=8.5,
        color=PALETTE["A"],
    ...
    # ── Insight text box ─────────────────────────────────────────────────
    insight = "Trees: 1.9–2.6×  |  CNN-LSTM: 2.1×  |  GRU: 2.4×  |  LSTM: 9.8× (failure)"
    ax.text(
        0.5,
        -0.13,
        insight,
        ...
        color="#444444",
        style="italic",
        bbox=dict(boxstyle="round,pad=0.3", facecolor="#fffbe6", edgecolor="#cccc88", alpha=0.85),
    )
